...
**APK 로 깔면 알림이 안 와요.** 우리 알림은 웹 푸시(서비스 워커 + VAPID)라
브라우저에만 있는 기능이거든요. 앱으로 깔면 그 자리가 "지금은 웹에서만
알림을 받을 수 있어요" 로 떠요. 터지지는 않아요.

알림까지 되게 하려면 expo-notifications + FCM 을 새로 붙여야 해요. 당장은
알림이 필요한 사람에게 **홈 화면에 추가**를 권하는 게 빨라요.

**권한은 인터넷·진동·사진뿐이에요.** 마이크와 '다른 앱 위에 그리기' 는
`app.json` 의 `blockedPermissions` 로 빼뒀어요. 사진 고르기 라이브러리가
동영상까지 지원하느라 마이크 권한을 같이 넣는데, 우리는 녹음을 안 해요.
학교에 나눠주는 앱에 마이크 권한이 붙어 있으면 그것만으로 안 깔아요.

**패키지 이름은 한 번 정하면 못 바꿔요.** `io.github.eoris1225.neischool`
이에요. 바꾸면 폰이 다른 앱으로 봐서, 깔려 있던 것 위에 안 덮여요.
