...
| `?kind=me` | 로그인한 내가 누구인지 |
| `?kind=promote` (POST) | 선생님으로 올리기 (코드 필요) |
...
로그인한 계정을 **선생님으로 올릴 때 한 번만** 써요. 통과하면 계정에 역할이
붙고, 그 뒤로는 로그인 자체가 권한이에요. 매 요청에 암호를 싣지 않아요.
...
수행평가 등록·삭제는 **로그인해서 선생님인 계정만** 할 수 있어요.
읽기는 누구나 돼요. 수행평가는 원래 학생이 다 보는 거니까요.

코드를 아는 사람은 누구나 선생님이 될 수 있다는 건 여전해요. 학교에서
선생님들께만 알려주고, 새면 바꿔주세요.
...
학생이 과목을 골라 질문하면 그 과목 선생님들이 받아요.

표가 둘이에요.

| 표 | 담는 것 |
|---|---|
| `threads` | 쪽지 한 묶음. 어느 학교·과목, 보낸 학생, 안 읽음 표시 |
| `messages` | 그 안에서 오간 한 줄 한 줄 |

둘 다 RLS를 켜고 **정책을 하나도 안 만들어요.** 앱 키로는 아무것도 못 봐요.
누가 무엇을 볼 수 있는지는 `_shared/threads.ts` 한 곳에서만 정해요.
...
수행평가는 앱이 보낸 `office`/`school`을 그대로 써요. 공개된 내용이라
누가 봐도 상관없거든요.

쪽지는 달라요. 앱이 보낸 값을 믿으면 **선생님이 설정에서 학교만 바꿔서
남의 학교 학생 질문을 읽을 수 있어요.** 그래서 쪽지는 `profiles` 표에
적힌 학교만 봐요. 앱은 학교를 고를 때마다 `kind=my-school`로 보내서
계정에 적어둬요.

### 볼 수 없는 쪽지는 "없다"고만 해요

`kind=thread&id=...` 로 남의 쪽지를 찔러봐도 "권한이 없어요"가 아니라
"그런 쪽지가 없어요"라고 답해요. 권한 오류를 주면 그 쪽지가 있다는 것까지
알려주는 셈이에요.

### 여는 순간 읽음이에요

따로 "읽음" 버튼을 만들지 않았어요. `kind=thread` 를 GET 하면 서버가
그 사람 쪽 안 읽음 표시를 끕니다.
